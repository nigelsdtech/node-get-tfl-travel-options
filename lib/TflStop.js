...
method.loadArrivals = function (params,cb) {
  ...
  tflRequest.get({
    ...
    self.log('info', 'Got TFL predictions - ' + JSON.stringify(arrivals))
    ...
    for (var i = 0; i < arrivals.length; i++) {
      arrivingVehicles.push({
        vehicleId: arrivals[i].vehicleId,
        timeToArrival: arrivals[i].timeToStation
      })
    }
